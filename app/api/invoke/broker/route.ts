...
function buildAuthHeaders(auth: InvokeAuthConfig | undefined): Record<string, string> {
  ...
  switch (auth.type) {
    case "apiKey": {
      ...
      if (!headerName || !value) {
        throw new Error("API key auth requires both header name and value.");
      }
    ...
    case "mulesoftClientIdSecret": {
      ...
      if (!clientId || !clientSecret) {
        throw new Error("MuleSoft auth requires client_id and client_secret.");
      }
      return {
        client_id: clientId,
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const authHeaders = buildAuthHeaders(body.auth);
