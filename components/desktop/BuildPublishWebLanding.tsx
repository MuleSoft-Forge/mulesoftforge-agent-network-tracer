...
const PRIVACY_POINTS = [
  ...
  {
    icon: Shield,
    title: "Worker credentials stay isolated",
    body: "Lifecycle execution runs in your worker environment with server-side credentials and queue isolation.",
  },
  {
    icon: Monitor,
    title: "Runs on dedicated lifecycle workers",
    body: "Build, publish, deploy, unpublish, and undeploy run on isolated lifecycle workers connected to your project queue.",
  },
] as const;
...
const LIFECYCLE_STEPS = [
  ...
  { icon: Upload, label: "Unpublish", detail: "Remove published assets (agent-fabric plugin v1.1.0+)" },
  { icon: Rocket, label: "Undeploy", detail: "Remove deployed network resources (agent-fabric plugin v1.1.0+)" },
] as const;
      ...
      <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)] lg:items-start">
        <div className="min-w-0">
          ...
          <p className="mt-3 text-sm leading-relaxed text-gray-600 sm:text-base">
            The full Agent Network lifecycle — build, publish, deploy, unpublish, and undeploy — runs through the
            Anypoint CLI on the lifecycle worker tier. Agent Network Studio in the browser stays as the control plane
            while execution happens on your configured build server infrastructure.
          </p>
          ...
          <div className="mt-8 rounded-xl border border-primary/20 bg-gradient-to-br from-primary/5 to-violet-500/5 p-4">
            <p className="text-sm font-medium text-gray-900">Run lifecycle jobs on your build server</p>
            <p className="mt-1 text-sm text-gray-600">
              Configure your deployment with Anypoint Connected App credentials in{" "}
              <code className="rounded bg-white/80 px-1 py-0.5 text-xs">.env.local</code>, then run{" "}
              <code className="rounded bg-white/80 px-1 py-0.5 text-xs">npm run worker</code>. Jobs are queued through
              Redis and executed by the worker process.
            </p>
            <Link
              href="/about"
        ...
        <figure className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-xl ring-1 ring-gray-100">
          <Image
            ...
            alt="Build and Publish in Agent Network Studio with lifecycle worker-backed CLI execution"
            width={1440}
            height={900}
          ...
          <figcaption className="border-t border-gray-100 bg-gray-50 px-4 py-2.5 text-xs text-gray-500">
            Build &amp; Publish via lifecycle workers — deploy options come from your project&apos;s exchange.json.
          </figcaption>
        </figure>
      </div>
