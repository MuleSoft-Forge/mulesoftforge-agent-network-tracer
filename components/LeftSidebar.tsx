...
import TasksList, { type TaskQueryHints } from "@/components/TasksList";
...
export default function LeftSidebar({
  ...
  useEffect(() => {
    ...
    if (current?.orgId) {
      setInitialOrgId(current.orgId);
      setSelectedEnvironmentId(current.envId ?? "");
    }
  ...
  const handleSelect = (value: string, allOrgIds: string[], rootOrgId: string) => {
    ...
    const restoredContext = readAnypointUiContext();
    const restoredEnvId =
      selection == null && restoredContext?.orgId === value ? (restoredContext.envId ?? "") : "";
    setSelection({ value, allOrgIds, rootOrgId });
    setSelectedEnvironmentId(restoredEnvId);
    setSelectedBrokerNodeId("");
    onBrokerChange?.(null);
    onExchangeNetworkChange?.(null);
    onEnvironmentChange?.("production");
    ...
    if (value !== "ALL") {
      onOrgAndEnvChange?.(value, restoredEnvId);
      writeAnypointUiContext(
        restoredEnvId ? { orgId: value, envId: restoredEnvId } : { orgId: value }
      );
    } else {
      onOrgAndEnvChange?.("", "");
      writeAnypointUiContext(null);
  ...
  const handleEnvironmentSelect = (envId: string, env: AnypointEnvironment | null) => {
    ...
    if (selection?.value && selection.value !== "ALL") {
      ...
      writeAnypointUiContext(
        newEnvId ? { orgId: selection.value, envId: newEnvId } : { orgId: selection.value }
      );
    }
  };
        ...
        >
          <div>
            <BusinessGroupSelector
              initialOrgId={initialOrgId || undefined}
