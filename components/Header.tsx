...
import { ChevronDown, LogOut, FileJson, RefreshCw, Trash2 } from "lucide-react";
...
import { clearLocalDesktopSettings } from "@/lib/desktop/clear-local-settings";
...
const NAV_ITEMS: NavItem[] = [
  ...
  { href: "/lifecycle", label: "Build & Publish", requiresAuth: true },
...
export default function Header() {
  ...
  async function handleSignOut(): Promise<void> {
    if (isDesktop()) {
      await getDesktop()?.auth.notifySignOut();
    }
  ...
  async function handleClearSettings(): Promise<void> {
    if (
      !window.confirm(
        "Clear saved sign-in and local preferences?\n\nThis removes your stored Anypoint credentials from the secure keychain, ends your session, and clears remembered project paths on this device."
      )
    ) {
      return;
    }
    ...
    if (isDesktop()) {
      await getDesktop()?.auth.clearAllSettings();
    }
    clearLocalDesktopSettings();
                    ...
                    <div className="py-1">
                      ...
                      {isDesktop() ? (
                        <button
                          ...
                          className="flex w-full items-center gap-2 px-3 py-2 text-left text-sm text-red-700 hover:bg-red-50 focus:bg-red-50 focus:outline-none"
                        >
