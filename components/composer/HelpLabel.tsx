...
const labelCls = "text-xs font-medium text-composer-label";
const uppercaseLabelCls = "text-xs font-semibold uppercase tracking-wide text-composer-label-muted";
  ...
  return (
    <div className={className}>
      <div className="mb-1 flex items-center gap-1">
        <span className={uppercase ? uppercaseLabelCls : labelCls}>
          {label}
          {required ? (
      ...
      {helpMode && help ? (
        <p className="mb-1 text-xs leading-snug text-primary/80">{help.tagline}</p>
      ) : null}
    </div>
  );
}
...
export function HelpSectionHeader({
  ...
  return (
    <div className="mb-1">
      ...
      {helpMode ? <p className="mt-0.5 text-xs leading-snug text-primary/80">{help.tagline}</p> : null}
    </div>
  );
}
...
export function HelpPanelIntro({ help, children }: { help: HelpEntry; children?: ReactNode }) {
  ...
  return (
    <div className="overflow-hidden rounded-anypoint border border-composer-border bg-composer-surface-muted/80">
      ...
      {open ? (
        <div className="space-y-1.5 border-t border-gray-100 px-3 py-2">
          ...
          {helpMode && help.whenToUse.length > 0 ? (
            <ul className="list-disc space-y-0.5 pl-4 text-xs text-composer-label-muted">
