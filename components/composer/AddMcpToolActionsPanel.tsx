...
import { mcpMetaForAsset, parseMcpAssetMeta } from "@/lib/composer/mcp-metadata";
...
function McpAssetToolAddRow({
  ...
  const catalog = meta?.tools ?? mcpMetaForAsset(asset)?.tools ?? parseMcpAssetMeta(asset.meta)?.tools ?? [];
