...
import { BookOpen, ChevronDown, ChevronRight, ExternalLink, HelpCircle, Lock, Trash2 } from "lucide-react";
...
import AgentScriptVariablesEditor from "@/components/composer/AgentScriptVariablesEditor";
...
import {
  ...
  isGraphPanelTab,
  type PanelTab,
} from "@/lib/composer/panel-tabs";
import { tabLock, type TabGate } from "@/lib/composer/tab-gating";
export {
  PANEL_TAB_GROUPS,
  isBrokerPanelTab,
  isGraphPanelTab,
...
export function ComposerNav({
  ...
  gate: TabGate;
  onOrderedTabsChange: (next: boolean) => void;
  onRequirementFocus: (target: ProjectFocusTarget) => void;
  ...
  return (
    <nav className="flex h-full w-[176px] shrink-0 flex-col overflow-hidden border-r border-composer-border bg-composer-surface">
      <div className="flex items-center gap-2 border-b border-composer-border px-3 py-2">
        <label className="flex items-center gap-2 text-[11px] text-composer-label-muted">
          <input
            type="checkbox"
            checked={gate.enabled}
            onChange={(event) => onOrderedTabsChange(event.target.checked)}
        ...
        <button
          ...
          title="Builds the network one step at a time: a tab unlocks once every earlier step holds the data it needs."
          aria-label="About the guided build order"
          className="rounded p-0.5 text-gray-400 transition-anypoint hover:bg-composer-surface-muted hover:text-primary"
        >
        ...
        {PANEL_TAB_GROUPS.map((group) => (
          <div key={group.title}>
            ...
            <div className="space-y-0.5">
              {group.tabs.map((t) => {
                const lock = tabLock(gate, t.id);
                const locked = lock !== null && tab !== t.id;
                return (
                  <button
                    key={t.id}
                    type="button"
                    ...
                    className={`${tabBtn} flex items-center gap-2 ${
...
export function ComposerPanelContent({
  ...
  const expressionCatalog = useMemo(
    () => (broker ? buildExpressionCatalog(broker) : []),
    [broker]
  );
  ...
  const policyDependencies = useMemo(() => {
    const rows: Array<{
      bindingName?: string;
      groupId: string;
      assetId: string;
  ...
  const policyDependencyRefs = useMemo(() => {
    const refs = new Set<string>();
    for (const dep of policyDependencies) {
      refs.add(`${dep.groupId}:${dep.assetId}`);
    }
    return refs;
  }, [policyDependencies]);
  const {
  ...
  function bumpPatchVersion(delta: number): void {
    const raw = project.identity.version.trim();
    const match = /^(\d+)\.(\d+)\.(\d+)$/.exec(raw);
    if (!match) return;
    const major = Number.parseInt(match[1], 10);
    const minor = Number.parseInt(match[2], 10);
    const patch = Number.parseInt(match[3], 10);
    if (!Number.isFinite(major) || !Number.isFinite(minor) || !Number.isFinite(patch)) return;
    const nextPatch = Math.max(0, patch + delta);
    dispatch({ type: "setIdentity", patch: { version: `${major}.${minor}.${nextPatch}` } });
  }
    ...
    <>
      <div className="flex h-full min-w-0 flex-col bg-composer-surface">
        ...
        <div className="min-h-0 flex-1 space-y-3 overflow-auto px-4 pt-4 pb-24 scrollbar-thin">
          {tab === "identity" && (
            <div className="flex flex-col gap-4 xl:flex-row">
              <div className="min-w-0 flex-1 space-y-6">
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    ...
                    <FieldAnchor id={PROJECT_ANCHOR.version}>
                      <TextField
                        ...
                        alwaysShowHint
                        trailing={
                          <div className="inline-flex overflow-hidden rounded border border-composer-border bg-white shadow-sm">
                            <button
                              type="button"
                              onClick={() => bumpPatchVersion(-1)}
                              disabled={!/^\d+\.\d+\.\d+$/.test(project.identity.version.trim())}
                              className="h-6 w-6 text-xs text-composer-label hover:bg-composer-surface-muted disabled:cursor-not-allowed disabled:opacity-40"
                              title="Decrement patch by 0.0.1"
                              aria-label="Decrement patch version"
                            >
                              -
                            </button>
                            <button
                              ...
                              onClick={() => bumpPatchVersion(1)}
                              disabled={!/^\d+\.\d+\.\d+$/.test(project.identity.version.trim())}
                              className="h-6 w-6 border-l border-composer-border text-xs text-composer-label hover:bg-composer-surface-muted disabled:cursor-not-allowed disabled:opacity-40"
                              title="Increment patch by 0.0.1"
                              aria-label="Increment patch version"
                            >
                              +
                            </button>
                          </div>
                ...
                <CollapsibleSection
                  title="Optional"
                  subtitle="Description, tags, and YAML info."
                  defaultOpen={false}
                >
                  <FormSection
                    title="Optional"
          ...
          {tab === "registry" && (
            <>
              <div className="rounded-anypoint border border-amber-300 bg-amber-50 px-3 py-2 text-xs leading-relaxed text-amber-900">
                <p className="font-semibold">Registry warning</p>
                <p className="mt-1">
                  MuleSoft Broker orchestrates Exchange-governed existing agentic assets. It should not be used to
                  create MCP servers or agents on the fly. Registry is included for legacy purposes.
                </p>
              </div>
              ...
              {broker && (
                <>
                  ...
                  <SelectField
                    ...
                    hint="Yaml brokers.*.interfaces map key. Builder projects intentionally edit one interface per broker; multi-interface imports are blocked to prevent loss."
                  />
                  <PolicyBindingsPanel
                    ...
                    policiesLoading={policiesLoading}
                    policiesError={policiesError}
                    allowedPolicyDependencyRefs={policyDependencyRefs}
                    ...
                    onEnsurePolicyDependency={(dependency) =>
                      dispatch({ type: "upsertUnmatchedDependency", dependency })
                    }
            ...
            <BrokerRequired broker={broker}>
              {broker && (
                <>
                  ...
                  />
                  <AgentScriptVariablesEditor broker={broker} />
                  <TextField
                    label="Agent dialect version"
                    value={broker.agentDialectVersion ?? "1.0"}
                    onChange={(agentDialectVersion) =>
                      dispatch({
                        type: "updateBroker",
                        patch: { agentDialectVersion: agentDialectVersion.trim() || undefined },
                      })
                    }
                    mono
                    help={helpForSection("field.agentDialectVersion")}
                    hint="AGENTFABRIC major.minor contract written to the .agent dialect header."
                  />
                  <TextField
                    label="Config label"
          ...
          {tab === "assets" && (
            <>
              ...
              <p className="text-[11px] text-gray-500">
                Use <strong>Compose from Exchange</strong> and select <strong>Policies</strong> to add policy dependencies.
              </p>
                ...
                <p className="text-xs text-gray-400">
                  {registryLocalAssetCount > 0
                    ...
                    : policyDependencies.length > 0
                      ? "No composed Exchange connections yet. Policy template dependencies are listed below."
              ...
              {policyDependencies.length > 0 ? (
                <div className="space-y-2 rounded-lg border border-gray-200 bg-gray-50/70 p-3">
                  <p className="text-xs font-semibold text-gray-700">
                    Policy template dependencies ({policyDependencies.length})
                  </p>
                  <p className="text-[11px] text-gray-500">
                  ...
                  <div className="space-y-2">
                    {policyDependencies.map((dep) => (
                      <div
                        key={`${dep.groupId}:${dep.assetId}:${dep.version}:${dep.classifier}:${dep.packaging}`}
                      ...
                      >
                        <div className="flex items-center gap-2 bg-gray-50 px-2 py-2">
                          ...
                          {dep.source === "imported" ? (
                            ...
                            <span className="mr-1 rounded bg-amber-100 px-1.5 py-0.5 text-[10px] font-semibold uppercase text-amber-800">
                              ref
                            </span>
                          )}
                        </div>
                          ...
                          {b.provider === "OpenAI" ? (
                            ...
                            <>
                              ...
                              <TextField
                                ...
                                hint="Zero disables thinking. The pinned validator currently rejects the documented -1 automatic mode."
                              />
                            </>
                          )}
                          <TextField
                          ...
                          <InstructionTextArea
                            ...
                            onChange={(headers) =>
                              dispatch({
                                type: "updateLlmBinding",
                                id: b.id,
                                patch: { headers: headers.trim() ? headers : undefined },
                              })
                            }
                            mono
                            rows={2}
                            catalog={expressionCatalog}
                            hint={'AgentScript map expression, e.g. {"X-Trace-Id": uuid()}.'}
                          />
                          <TextField
                            label="Timeout (seconds)"
                            value={b.timeout !== undefined ? String(b.timeout) : ""}
                            onChange={(value) => {
                              const trimmed = value.trim();
                              dispatch({
                                type: "updateLlmBinding",
                                id: b.id,
                                patch: { timeout: trimmed ? Number(trimmed) : undefined },
                              });
                            }}
                          />
                          <TextField
                            ...
                            onChange={(apiKey) =>
                              dispatch({
                                type: "updateLlmBinding",
                                id: b.id,
                                patch: { apiKey: apiKey.trim() ? apiKey : undefined },
                              })
                            }
                            mono
                            hint="Optional provider API key. Prefer connection authentication when available."
                          />
                          {b.provider === "OpenAI" ? (
                            <TextField
                              label="Top logprobs"
                            ...
                            <SelectField
                              ...
                              value={
                                b.responseLogprobs === undefined
                                  ? ""
                                  : String(b.responseLogprobs)
                              }
                              options={[
                                { value: "", label: "(default)" },
