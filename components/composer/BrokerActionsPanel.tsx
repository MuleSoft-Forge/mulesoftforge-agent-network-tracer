        ...
        <div key={index} className="flex items-end gap-2">
          ...
          <div className="w-24 shrink-0">
            <TextField
              ...
              value={inp.type}
              onChange={(value) =>
                onChange(inputs.map((row, i) => (i === index ? { ...row, type: value } : row)))
              }
              mono
            />
          </div>
          <div className="min-w-0 flex-1">
            <TextField
              ...
              onChange={(value) =>
                onChange(
                  inputs.map((row, i) =>
                    i === index ? { ...row, default: value.trim() ? value : undefined } : row
                  )
                )
              }
              mono
...
function HttpHeadersEditor({
  ...
  onChange: (next: Array<{ name: string; value: string }> | undefined) => void;
}) {
  return (
    <div className="space-y-1.5 rounded-md border border-dashed border-gray-200 bg-gray-50/80 p-2">
      <div className="flex items-center justify-between gap-2">
        <p className="text-[10px] font-medium uppercase tracking-wide text-gray-500">HTTP headers</p>
        <Button variant="ghost" onClick={() => onChange([...headers, { name: "Header-Name", value: "" }])}>
          <Plus className="h-3 w-3" /> Add
        </Button>
      </div>
        ...
        <div key={index} className="flex items-end gap-2">
          <div className="w-36 shrink-0">
            <TextField
              label="Name"
              value={header.name}
              onChange={(value) => onChange(headers.map((row, i) => (i === index ? { ...row, name: value } : row)))}
              mono
            />
          </div>
          <div className="min-w-0 flex-1">
            <TextField
              label="Value"
              value={header.value}
              onChange={(value) => onChange(headers.map((row, i) => (i === index ? { ...row, value } : row)))}
              mono
            />
          </div>
          <Button
    ...
    <div className="space-y-2 rounded-md border border-gray-100 bg-white p-2">
      ...
      {action.actionKind === "mcp:tool" ? (
        <ActionInputsEditor
          ...
          onChange={(inputs) => dispatch({ type: "updateAction", id: action.id, patch: { inputs } })}
        />
      ) : null}
      <HttpHeadersEditor
        headers={action.httpHeaders ?? []}
        onChange={(httpHeaders) => dispatch({ type: "updateAction", id: action.id, patch: { httpHeaders } })}
      />
      {action.actionKind === "mcp:tool" && (
        <McpActionToolField
          action={action}
