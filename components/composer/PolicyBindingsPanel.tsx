...
function PolicyBindingList({
  ...
  function commitRefs(nextRefs: Array<{ mode: "ref"; name: string; namespace?: string }>) {
    const valid: Array<{ mode: "ref"; name: string; namespace?: string }> = [];
    let dropped = false;
    for (const ref of nextRefs) {
      const ns = ref.namespace?.trim();
      const name = ref.name.trim();
      if (!name) continue;
      const depRefKey = `${ns ?? organizationId}:${name}`;
      const fromCatalog = Boolean(findPolicyOption(exchangePolicies, { name, ...(ns ? { namespace: ns } : {}) }));
      if (fromCatalog || allowedPolicyDependencyRefs.has(depRefKey)) {
        valid.push({ mode: "ref", name, ...(ns ? { namespace: ns } : {}) });
      } else {
        dropped = true;
      }
    }
    if (dropped) {
      setDraftError("One or more policy refs were removed because matching policy dependencies are missing.");
    } else {
      setDraftError(null);
    }
    commitAll(valid, inlines);
  }
  ...
  function pickPolicy(picked: ExchangePolicyOption) {
    if (!picked.version) {
      setDraftError(`Policy template "${picked.name}" is missing a version and cannot be referenced.`);
      return;
    }
  ...
  function commitCustomDraft(draft: DraftPolicyRow) {
    ...
    const depRefKey = `${namespace ?? organizationId}:${name}`;
    if (!allowedPolicyDependencyRefs.has(depRefKey)) {
      setDraftError(
        `Add dependency first for ${depRefKey} under Exchange Assets before creating this policy ref.`
      );
      return;
    }
  ...
  return (
    <div className="space-y-1.5">
      ...
      {draftError ? <p className="text-[11px] text-red-600">{draftError}</p> : null}
      ...
      {refs.map((item, index) => {
        ...
        return (
          <div key={`policy-ref-${index}`} className="flex items-start gap-1.5 rounded border border-gray-200 bg-gray-50/80 p-2">
            <div className="min-w-0 flex-1 space-y-1.5">
              <SelectField
