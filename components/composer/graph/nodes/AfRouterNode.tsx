...
import { NodeCoachFooter } from "@/components/composer/graph/nodes/NodeCoach";
import { useNodeDetailLevel } from "@/components/composer/graph/nodes/use-node-detail-level";

const ROUTER_ACCENT = "#f59e0b";

const ROUTE_SLOT_HANDLE = routerOutputHandleId(ROUTER_ROUTE_SLOT);
const OTHERWISE_SLOT_HANDLE = routerOutputHandleId(ROUTER_OTHERWISE_SLOT);

function routerOutputDisplayLabel(output: RouterCanvasOutput): string {
  if (output.handleId === ROUTE_SLOT_HANDLE) return "+ route";
  return output.label;
}

function isRouterSlotHandle(handleId: string): boolean {
  return handleId === ROUTE_SLOT_HANDLE || handleId === OTHERWISE_SLOT_HANDLE;
}

/** Read-only protocol graphs carry labels only, so fall back to label handles. */
function outputsFromProtocolBag(outputs: string | undefined): RouterCanvasOutput[] {
  return parseProtocolOutputs(outputs).map((label) => ({
    handleId: routerOutputHandleId(label),
    label,
  }));
}

export function AfRouterNode({ data, selected }: NodeProps<Node<AgentFabricGraphNodeData>>) {
  const outputs = data.routerOutputs ?? outputsFromProtocolBag(data.outputs);
  const connected = data.connectedHandles;
  const compatibility = data.handleCompatibility ?? {};
  const showDetail = useNodeDetailLevel() === "full";
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-3 px-4 py-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            <NodeIssueDot
              severity={data.issueSeverity}
              summary={data.issueSummary}
              onClick={data.onOpenCoach}
            />
          </div>
          ...
          {outputs.length > 0 ? (
            <ul className="mt-2 space-y-1">
              {outputs.map((output) => {
                const handleId = output.handleId;
                const isConnected = connected?.has(handleId) ?? false;
                const isSlot = isRouterSlotHandle(handleId);
                ...
                return (
                  ...
                  >
                    <span className="truncate" title={output.label}>
                      {routerOutputDisplayLabel(output)}
                    </span>
                    <Handle
                      id={handleId}
                      type="source"
      ...
      {showDetail && data.coach ? <NodeCoachFooter coach={data.coach} /> : null}
    </div>
  );
}
