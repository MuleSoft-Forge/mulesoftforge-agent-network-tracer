...
export type {
  ...
  NodeToolCall,
  V2NodeTimeline,
} from "@/lib/task-timeline/build-v2-node-timeline";
...
export interface ApiStatus {
  ...
  monitoringSuggestions?: { insecureLogging: boolean };
}

/** `graph` is v2-only: v1 brokers ran a ReAct loop, not a node graph. */
export type ViewMode = "tree" | "list" | "graph";
export type DetailTab = "apiStatus" | "story" | "message" | "metadata" | "traces" | "reasoning" | "raw";

export interface SelectedItem {
  type: "task" | "iteration" | "step" | "tool" | "node";
  id: string;
  data: unknown;
}
