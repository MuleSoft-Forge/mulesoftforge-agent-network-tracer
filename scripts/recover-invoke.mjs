#!/usr/bin/env node
/** Replay Write/StrReplace for invoke-related files from agent transcripts. */
...
const TARGET_RE =
  /^(?:components\/invoke\/|lib\/invoke\/|app\/api\/invoke\/|lib\/api\/url-safety\.ts$)/;
...
/** Collapse duplicate consecutive import lines (replay can apply the same StrReplace twice). */
function dedupeConsecutiveImports(source) {
  const lines = source.split("\n");
  const out = [];
  for (const line of lines) {
    const prev = out[out.length - 1];
    if (prev !== undefined && line === prev && line.startsWith("import ")) continue;
    out.push(line);
  }
  return out.join("\n");
}
...
for (const [r, content] of files.entries()) {
  files.set(r, dedupeConsecutiveImports(content));
}
...
console.log(`Done: ${written} invoke files updated (${ops.length} ops replayed).`);
