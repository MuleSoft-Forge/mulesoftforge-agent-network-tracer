 ...
 *
 * Default source (when env unset): sibling MuleSoft-Agent-Network-Project checkout.
 */
...
const DEFAULT_SOURCE = resolve(
  REPO_ROOT,
  "../MuleSoft-Agent-Network-Project/source/agent-fabric-specification"
);
...
function main() {
  const sourceRoot = resolve(process.env.ANF_SCHEMA_SOURCE ?? DEFAULT_SOURCE);
  const resourcesDir = join(sourceRoot, ANF_BUNDLE_SOURCE.subpath);

  if (!existsSync(resourcesDir)) {
    console.error(`Schema source not found: ${resourcesDir}`);
    console.error("Set ANF_SCHEMA_SOURCE to your agent-fabric-specification checkout.");
    process.exit(1);
  }
