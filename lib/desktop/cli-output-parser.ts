...
function parseStructuredLine(line: string): CliActivityItem | null {
  ...
  const start = line.match(/^Deployment for (?:connection|instance|Agent Graph): '([^']+)' starting\.\.\.$/);
  if (start) {
  ...
  const finished = line.match(/^Deployment for (?:connection|instance|Agent Graph): '([^']+)' finished\.\.\./);
  if (finished) {
