...
export interface LogSearchParams {
  ...
  timeRangeMs?: number;
  /**
   * Restrict `_source` to these fields. Task discovery pages through thousands
   * of documents but only reads a handful of fields, and the untrimmed document
   * also carries `class`, `workerId`, `source` and `event` — so trimming here
   * cuts the transferred payload several-fold. Omit to fetch the full document.
   */
  sourceFields?: string[];
  /**
   * Anypoint environment id. `envId` is a mapped, aggregatable keyword on the
   * monitoring index, so this becomes a real shard-side filter — the same one
   * the Monitoring UI sends. Measured on a live org: 235,282 documents in a
   * 3-day window fell to 95,101 once this was applied.
   */
  envId?: string;
}
 ...
 */
function buildQuery(luceneQuery: string, gte: number, lte: number, envId?: string): unknown {
  ...
  if (envId != null && envId.trim() !== "") {
    filter.push({ match_phrase: { envId: envId.trim() } });
  }
      ...
      ? [
          {
            query_string: {
              ...
              // `message` is an analyzed text field, so wildcard terms only match
              // the lowercased tokens in the index unless they are analyzed too.
              // Without this, `message:*Some_Route*` silently matches nothing.
...
export async function logSearch(params: LogSearchParams): Promise<LogSearchResult> {
  ...
  // `stored_fields`, `version` and `docvalue_fields` exist to mirror what the
  // Monitoring UI sends; none of our parsers read them, so they are dropped
  // whenever the caller has declared the fields it needs.
  const trimmed = sourceFields != null && sourceFields.length > 0;
  ...
  const body = {
    params: {
      ...
      body: {
        ...
        query: buildQuery(luceneQuery, now - timeRangeMs, now, envId),
      },
      preference: now,
    },
