...
async function findObjectStore(
  ...
  // Try v1 prefixes, then v2 Object-Store name
  for (const prefix of [storeIdPrefixShort, storeIdPrefixFull, storeIdV2]) {
...
function decodeObjectStoreStrings(value: ObjectStoreValue, brokerFormat: ObjectStoreBrokerFormat): string[] {
  if (value.valueType === "STRING" && typeof value.stringValue === "string") {
    return extractStringsFromV2StorageEntry(value.stringValue);
  }
  ...
  if (typeof value.binaryValue === "string") {
    return brokerFormat === "v2"
      ? extractStringsFromV2StorageEntry(
          Buffer.from(value.binaryValue, "base64").toString("utf8")
        )
      : extractStringsFromBinary(value.binaryValue);
  }
  return [];
}
...
async function findTaskKey(
  ...
  // v2: exact key match first (cheaper than listing all keys)
  const exact = await getPartitionValue(orgId, envId, storeId, partitionId, taskId, accessToken, region);
  if (exact) return taskId;

  return findKeyContainingTaskId(orgId, envId, storeId, partitionId, taskId, accessToken, region);
}
...
async function fetchValueFromPartition(
  ...
  const foundKey = await findTaskKey(orgId, envId, storeId, partitionId, taskId, accessToken, region);
  if (foundKey) {
    const value = await getPartitionValue(orgId, envId, storeId, partitionId, foundKey, accessToken, region);
    if (value) {
      debugLog(`[ObjectStore] Found value in partition "${partitionId}" with key: ${foundKey}`);
      const resolvedFormat = brokerFormat === "unknown" ? partitionBrokerFormat(partitionId) : brokerFormat;
      return { value, keyUsed: foundKey, partitionId, brokerFormat: resolvedFormat };
    }
  }
...
export async function fetchObjectStoreData(
  ...
  // store attachment), but brokers can still write task state to ephemeral Object Store
  // with ~24h TTL — we rely on tenant/deployment resolution + task age instead of skipping.
  ...
  // v2: supplement task-store text with graph-state-store when task history is sparse
  if (tasksValue && resolvedFormat === "v2" && tasksStrings.length < 3 && partitions.graphStatePartitionCandidates.length > 0) {
    for (const graphPartition of partitions.graphStatePartitionCandidates) {
      try {
  ...
  const hasAnyReasoning = steps.length > 0 || rawReasoning.length > 0 || tasksStrings.length > 0;
  ...
  return {
    ...
    llmReasoning: hasAnyReasoning ? {
      ...
      rawReasoning: rawReasoning.length > 0 ? rawReasoning : tasksStrings,
