...
export async function runSimulation(
  ...
  dispatch({ type: "SET_PROCESSING", value: true, step: "Simulating (no broker URL)…" });
  dispatch({ type: "RESET_NODE_STATUSES" });
  ...
  dispatch({
    ...
    step: `Simulating ${targetNodes.map((n) => n.label).join(", ")}…`,
  });
...
export async function handleSend(
  ...
  if (brokerUrl.trim()) {
    return callRealBroker(userMessage, brokerUrl, graph, dispatch, a2aVersion);
  }
