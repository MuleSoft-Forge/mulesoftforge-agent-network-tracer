...
/**
 * For a given skill name, find the best matching canonical node ID in `graph`.
 * Returns the nodeId to highlight when that skill is selected, or undefined if no match.
 */
export function findCanonicalNodeForSkill(
  skillName: string,
  graph: CanonicalGraph
): string | undefined {
  const lower = skillName.toLowerCase().replace(/\bskill\b/g, "").trim();
  const subNodes = graph.nodes.filter(
    (n) => n.type === "AGENT" || n.type === "MCP"
  );

  // Exact label match (case-insensitive)
  const exact = subNodes.find((n) =>
    n.label.toLowerCase().replace(/\bskill\b/g, "").trim() === lower
  );
  if (exact) return exact.id;

  // Word-overlap scoring
  const queryWords = lower.split(/\s+/).filter((w) => w.length > 2);
  let best: CanonicalNode | undefined;
  let bestScore = 0;
  for (const n of subNodes) {
    const nodeWords = n.label.toLowerCase().split(/\s+/).filter((w) => w.length > 2);
    const score = queryWords.filter((w) => nodeWords.some((nw) => nw.includes(w) || w.includes(nw))).length;
    if (score > bestScore) {
      bestScore = score;
      best = n;
    }
