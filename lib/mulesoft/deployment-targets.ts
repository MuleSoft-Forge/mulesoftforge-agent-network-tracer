...
export async function fetchDeploymentTargets(
  ...
  for (const url of urls) {
    try {
      ...
      if (!res.ok) {
        debugLog("[DEPLOYMENT-TARGETS] source failed", { url, status: res.status });
        continue;
      }
      const parsed = parseDeploymentTargetsResponse(await res.json());
      debugLog("[DEPLOYMENT-TARGETS] source parsed", {
        url,
        count: parsed.length,
        targets: parsed.map((t) => ({ name: t.name, kind: t.kind })),
      });
      if (parsed.length > 0) return parsed;
    } catch {
      debugLog("[DEPLOYMENT-TARGETS] source threw", { url });
      // try next source
    }
  }

  debugLog("[DEPLOYMENT-TARGETS] no targets from any source", {
    orgId,
    envId,
