...
/** Parse a plain `groupId:assetId:version` coordinate string (e.g. from a GAV picker). */
export function parseGav(gav: string): Gav | undefined {
  if (typeof gav !== "string") return undefined;
  const parts = gav.trim().split(":");
  if (parts.length !== 3) return undefined;
  const [groupId, assetId, version] = parts;
  if (!groupId || !assetId || !version) return undefined;
