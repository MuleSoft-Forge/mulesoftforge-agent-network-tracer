...
export const TaskCallstackRequestSchema = z.object({
  ...
  // Hints carried over from the already-loaded task row. They only narrow the
  // log-search query — the route stays correct without them — so they are
  // optional and are validated strictly because `appId` is interpolated into a
  // Lucene clause.
  appId: z
    .string()
    .min(1)
...
export const ProfileSchema = z.object({
  ...
  /**
   * Decided server-side by `/api/auth/profile` so the allowlist itself never
   * reaches the browser bundle. Nav visibility only — every /api/ops route
   * re-checks it against the session token.
   */
