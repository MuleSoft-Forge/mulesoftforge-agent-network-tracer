...
export interface BrokerContext {
  ...
  /** Parsed identity of the owning Agent Network from RM metadata.source URN. */
  sourceMetadata?: {
    urn: string;
    groupId: string;
    assetId: string;
...
/**
 * Cached by (org, env, apiInstance, token). Deployment topology barely changes
 * within a session, yet both `/api/broker-tasks` and every `/api/task-callstack`
 * click resolved it again — two to three Anypoint round trips each. The promise
 * itself is cached, so concurrent callers share one in-flight resolution rather
 * than racing. The token is part of the key so a caller never inherits a result
 * resolved under someone else's permissions.
 */
const contextCache = new Map<string, { value: Promise<BrokerContext | null>; at: number }>();
...
export function resolveBrokerContext(
  ...
  const key = `${baseUrl}|${orgId}|${envId}|${apiInstanceId}|${accessToken}`;
  const cached = contextCache.get(key);
  if (cached && Date.now() - cached.at < CONTEXT_TTL_MS) {
    debugLog(`[BROKER-CTX] Cache hit for apiInstanceId=${apiInstanceId}, envId=${envId}`);
    return cached.value;
  }

  const pending = resolveBrokerContextUncached(
    orgId,
    envId,
    apiInstanceId,
    accessToken,
    ...
    // A failed resolution must not be cached, or one transient 5xx poisons the
    // broker for the whole TTL.
  ...
  contextCache.set(key, { value: pending, at: Date.now() });
  return pending;
}

async function resolveBrokerContextUncached(
  orgId: string,
  envId: string,
  apiInstanceId: string,
  accessToken: string,
  ...
  const parsedSource = parseGavFromMetadataSource(apiInstanceInfo.metadata?.source);
  ...
  if (amcMatch) {
    ...
    return buildBrokerContext(
      ...
      rmFields
      ,
      apiInstanceInfo.metadata?.source,
      parsedSource
    );
  }
...
function buildBrokerContext(
  ...
  parsedSource?: { groupId: string; assetId: string; version: string }
): BrokerContext {
  ...
  return {
    ...
    ...(metadataSourceUrn && parsedSource
      ? {
          sourceMetadata: {
            urn: metadataSourceUrn,
            groupId: parsedSource.groupId,
            assetId: parsedSource.assetId,
