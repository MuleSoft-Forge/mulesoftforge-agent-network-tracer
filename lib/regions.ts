...
/**
 * Regions offered on the sign-in screen. Desktop uses password login against any
 * available control plane; web requires Connected App credentials per region.
 */
export function getSignInRegionIds(): RegionId[] {
  if (isElectronDesktop()) {
    return getAvailableRegions().map((r) => r.id);
  }
