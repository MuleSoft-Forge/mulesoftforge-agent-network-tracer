...
export function runCli(options: RunCliOptions): Promise<RunCliResult> {
  ...
  // Inherit the ambient env (PATH, HOME, …), but first drop any conflicting auth
  // vars so the resolved credentials are the only auth the CLI sees.
  ...
  // process wedged in a syscall may ignore SIGTERM, and leaving it alive would
  ...
  const collect = (stream: NodeJS.ReadableStream | null, channel: "stdout" | "stderr") => {
    ...
    stream.on("data", (chunk: string) => {
      ...
      // Keep only a bounded tail: enough for the final JSON payload and error
      // scanning, without letting a chatty or looping CLI grow unbounded.
  ...
  // `--json` steps print nothing until they finish, so report liveness to keep
  // the activity view from looking stalled.
