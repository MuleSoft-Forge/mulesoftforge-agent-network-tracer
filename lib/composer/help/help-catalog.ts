...
const NODE_HELP: Record<GraphNodeKind, HelpEntry> = {
  ...
  on_exit: -> transition to @orchestrator.main`,
    ...
    gotchas: [
      "do is required",
      "Slot filling (...) is not allowed because no LLM resolves arguments",
      "Builder validation requires reachable paths to continue to a terminal status echo",
    ],
  ...
  message: @generator.summarize.output`,
    ...
    gotchas: [
      ...
      "Artifact and progress events must transition onward to a terminal status event",
    ],
