...
/** Reserved handle suffixes for the router's "add route" and fallback slots. */
export const ROUTER_ROUTE_SLOT = "route";
export const ROUTER_OTHERWISE_SLOT = "otherwise";

/** Human-readable text shown against a route's handle. */
export function routeOutputLabel(route: { label?: string; when: string }): string {
  return (route.label || route.when || ROUTER_ROUTE_SLOT).trim();
}

/**
 * A router output on the Builder canvas. The handle is keyed by route id, not
 * by label: two routes may legitimately share a condition or a name, and using
 * the label as identity collides their handles, edges, and edits.
 */
export interface RouterCanvasOutput {
  handleId: string;
  label: string;
}
...
export function routerCanvasOutputs(node: {
  routes?: Array<{ id: string; label?: string; when: string }>;
}): RouterCanvasOutput[] {
  const outputs: RouterCanvasOutput[] = (node.routes ?? []).map((route) => ({
    handleId: routerOutputHandleId(route.id),
    label: routeOutputLabel(route),
  }));
  outputs.push({ handleId: routerOutputHandleId(ROUTER_ROUTE_SLOT), label: ROUTER_ROUTE_SLOT });
  outputs.push({
    handleId: routerOutputHandleId(ROUTER_OTHERWISE_SLOT),
    label: ROUTER_OTHERWISE_SLOT,
  });
  return outputs;
}
