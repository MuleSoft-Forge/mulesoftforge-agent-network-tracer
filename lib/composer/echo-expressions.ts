...
export function emitStatusMessageLines(message: string, baseIndent: number): string[] {
  ...
  if (!partsMatch) return [`${pad}message: ${m}`];
...
/** Emit indented `task:` lines for a2a:response echo nodes. */
export function emitTaskLines(task: string, baseIndent: number): string[] {
  const pad = " ".repeat(baseIndent);
  const t = task.trim();
  if (!t.startsWith("a2a.task(")) return [`${pad}task: ${t}`];

  // Best-effort pretty-print when the expression matches the common task shape.
  const messageMatch = t.match(
    /a2a\.task\(\{\s*state:\s*"([^"]+)",\s*message:\s*(a2a\.message\(\{[\s\S]+\}\)),\s*metadata:\s*(\w+)\s*\}\)/
  );
  if (!messageMatch) return [`${pad}task: ${t}`];

  const [, state, messageExpr, metadata] = messageMatch;
  const msgLines = emitStatusMessageLines(messageExpr, baseIndent + 2);
  const out = [`${pad}task: a2a.task({`, `${pad}  state: ${JSON.stringify(state)},`];
  out.push(`${pad}  message: ${msgLines[0].trim().replace(/^message:\s*/, "")}`);
  for (let i = 1; i < msgLines.length; i++) out.push(msgLines[i]);
  out.push(`${pad}  metadata: ${metadata}`, `${pad}})`);
  return out;
}
