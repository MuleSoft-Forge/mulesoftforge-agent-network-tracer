...
function parseBroker(brokers: Record<string, unknown> | undefined): ParsedYamlBroker | undefined {
  ...
  if (entries.length > 1) {
    throw new Error(
      `Builder import supports exactly one broker; this project defines ${entries.length}. No broker was discarded.`
    );
  }
  ...
  if (ifaceEntries.length > 1) {
    throw new Error(
      `Builder import supports one interface per broker; broker "${key}" defines ${ifaceEntries.length}. No interface was discarded.`
    );
  }
  ...
  return {
    key,
    ...(asString(entry?.implementation) ? { implementation: asString(entry?.implementation) } : {}),
    interfaceName,
