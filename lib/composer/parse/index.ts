...
function toGraphNode(pn: ParsedGraphNode, id: string, position: { x: number; y: number }): GraphNode {
  ...
  if (pn.reasoningInstructionsProcedureInline) node.reasoningInstructionsProcedureInline = true;
  ...
  if (pn.promptProcedureInline) node.promptProcedureInline = true;
...
function buildNodes(parsed: ParsedBrokerAgent): GraphNode[] {
  ...
  parsed.nodes.forEach((pn, i) => referenceToId.set(`${pn.kind}:${pn.name}`, ids[i]));
  const resolveTarget = (target: { kind: string; name: string }): string | undefined =>
    referenceToId.get(`${target.kind}:${target.name}`);

  return parsed.nodes.map((pn, i) => {
    const node = toGraphNode(pn, ids[i], layoutPosition(i));
    if (pn.onExitTarget) {
      const target = resolveTarget(pn.onExitTarget);
    ...
    if (pn.otherwiseTarget) {
      const target = resolveTarget(pn.otherwiseTarget);
...
function buildLlmBindings(parsed: ParsedBrokerAgent): LlmBinding[] {
  return parsed.llmBindings.map((b) => ({
    ...
    ...(b.timeout !== undefined && Number.isFinite(b.timeout) ? { timeout: b.timeout } : {}),
...
function migrateLegacyActionHeaders(parsed: ParsedBrokerAgent, nodes: GraphNode[]): string[] {
  const warnings: string[] = [];
  for (const action of parsed.actions) {
    if (!action.httpHeaders?.length) continue;
    const value = JSON.stringify(Object.fromEntries(action.httpHeaders.map((header) => [header.name, header.value])));
    let migrations = 0;
    for (const node of nodes) {
      if (node.actionBindings) {
        node.actionBindings = node.actionBindings.map((binding) => {
          if (binding.actionName !== action.name || binding.withArgs?.some((arg) => arg.name === "http_headers")) {
            return binding;
          }
          migrations++;
          return {
            ...binding,
            withArgs: [...(binding.withArgs ?? []), { name: "http_headers", value }],
          };
        });
      }
      if (node.executorStatements) {
        node.executorStatements = node.executorStatements.map((statement) => {
          if (
            statement.kind !== "run" ||
            statement.actionName !== action.name ||
            statement.withArgs?.some((arg) => arg.name === "http_headers")
          ) {
            return statement;
          }
          migrations++;
          return {
            ...statement,
            withArgs: [...(statement.withArgs ?? []), { name: "http_headers", value }],
          };
        });
      }
    }
    warnings.push(
      migrations > 0
        ? `Action "${action.name}": moved legacy action-level http_headers to ${migrations} invocation binding${migrations === 1 ? "" : "s"}.`
        : `Action "${action.name}": removed unsupported action-level http_headers because the action has no invocation to migrate.`
    );
  }
  return warnings;
}
...
export function parseProjectFiles(input: ParseFilesInput): ParseFilesResult {
  ...
  const hasBrokerSource = Boolean(yamlDoc.broker || input.brokerAgent?.trim());
  const rawBrokerName =
    agent.agentName ??
    yamlDoc.broker?.key ??
    yamlDoc.broker?.card.name ??
    exchange.assetId ??
    "";
  const brokerName = isValidBrokerKey(rawBrokerName)
    ? rawBrokerName
    : normalizeBrokerKey(rawBrokerName, "");

  const nodes = buildNodes(agent);
  warnings.push(...migrateLegacyActionHeaders(agent, nodes));
  const broker: Broker = {
    id: newId(),
    name: brokerName,
    interfaceName: yamlDoc.broker?.interfaceName ?? "a2a",
    card: yamlDoc.broker?.card ?? { name: brokerName, version: "1.0.0" },
    ...
    ...(agent.systemInstructionsProcedureInline ? { systemInstructionsProcedureInline: true } : {}),
    ...
    agentScriptVariables: agent.agentScriptVariables,
  ...
  const identityAssetId =
    exchange.assetId ?? yamlDoc.broker?.key ?? agent.agentName ?? "";
  const candidate: ComposerProject = {
    version: 1,
    identity: {
      name: yamlDoc.label ?? exchange.name ?? identityAssetId,
    ...
    assets,
    brokers: hasBrokerSource ? [brokerWithLayout] : [],
