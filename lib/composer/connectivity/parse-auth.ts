  ...
  switch (kind) {
    ...
    case "in-task-authorization-code": {
      ...
      return {
        ...
        ...(asString(raw.codeChallengeMethod) ? { codeChallengeMethod: asString(raw.codeChallengeMethod) } : {}),
        ...(asString(raw.tokenAudience) ? { tokenAudience: asString(raw.tokenAudience) } : {}),
        ...(asString(raw.bodyEncoding) === "form" || asString(raw.bodyEncoding) === "json"
          ? { bodyEncoding: asString(raw.bodyEncoding) as "form" | "json" }
          : {}),
        ...(asNumber(raw.tokenTimeout) !== undefined ? { tokenTimeout: asNumber(raw.tokenTimeout) } : {}),
        ...(asNumber(raw.challengeResponseStatusCode) !== undefined
          ? { challengeResponseStatusCode: asNumber(raw.challengeResponseStatusCode) }
          : asNumber(raw.challengeStatusCode) !== undefined
            ? { challengeResponseStatusCode: asNumber(raw.challengeStatusCode) }
            : {}),
