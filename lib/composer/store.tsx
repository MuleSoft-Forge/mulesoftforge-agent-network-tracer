 ...
 * a route for the handle that was dragged; echo nodes are terminal and unchanged.
 */
  ...
  return {
    ...
    nodes: broker.nodes.map((n) => {
      ...
      if (n.kind === "router") {
        ...
        const output = routerOutputFromHandleId(sourceHandle);
        if (output === "otherwise") {
          return { ...n, otherwiseTargetNodeId: targetId };
        }
        if (output) {
          const existingIdx = routes.findIndex((route) => routeOutputLabel(route) === output);
          if (existingIdx >= 0) {
  ...
  switch (action.type) {
    ...
    case "addNode":
      return updateBroker(project, (b) => {
        ...
        return autoWireTriggerOnAdd(withNode, node, action.connectFrom);
      });
    ...
    case "insertNodeOnEdge":
      return updateBroker(project, (b) => {
        ...
        return connectNodes(rewired, node.id, action.targetId, null);
      });
    ...
    case "disconnect":
      return updateBroker(project, (b) => ({
        ...
        nodes: b.nodes.map((n) => {
          ...
          if (n.kind === "router") {
            const routes = n.routes ?? [];
            const output = routerOutputFromHandleId(action.sourceHandle);
            if (output === "otherwise") {
            ...
            if (output && output !== "route") {
              return {
                ...n,
                routes: routes.filter((route) => routeOutputLabel(route) !== output),
              };
            }
            return {
              ...n,
