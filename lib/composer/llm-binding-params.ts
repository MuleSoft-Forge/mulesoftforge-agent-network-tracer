...
export function llmTuningYamlEntries(binding: LlmBinding): Array<[string, Scalar]> {
  ...
  if (binding.responseLogprobs !== undefined) {
    // The pinned AgentFabric dialect models this provider flag as a string
    // value, even though the public reference describes boolean semantics.
    entries.push(["response_logprobs", String(binding.responseLogprobs)]);
  }
  return entries;
}
