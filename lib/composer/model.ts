...
export const OutputValueSchema = z.union([z.string(), z.number(), z.boolean()]);
...
export type OutputSchemaNode = {
  ...
  description?: string;
  default?: OutputValue;
  /** Allowed literal values for the output. */
  enum?: OutputValue[];
  pattern?: string;
  minLength?: number;
  maxLength?: number;
  ...
  /** Recursively nested item schema when type is array. */
  items?: OutputSchemaNode;
};

export type OutputProperty = OutputSchemaNode & {
  name: string;
};

export const OutputSchemaNodeSchema: z.ZodType<OutputSchemaNode> = z.lazy(() =>
  z.object({
    type: z.enum(["string", "number", "integer", "boolean", "array", "object"]),
    description: z.string().optional(),
    default: OutputValueSchema.optional(),
    enum: z.array(OutputValueSchema).optional(),
    ...
    items: OutputSchemaNodeSchema.optional(),
  })
);

export const OutputPropertySchema: z.ZodType<OutputProperty> = z.lazy(() =>
  OutputSchemaNodeSchema.and(z.object({ name: z.string().min(1) }))
);
...
export const ExecutorRunStatementSchema = z.object({
  ...
  /** Assignments nested under this run; expressions can reference `@outputs`. */
  captures: z.array(ExecutorSetStatementSchema).optional(),
});
...
export const AgentScriptVariableSchema = z.object({
  name: z.string().min(1),
  modifier: z.enum(["mutable", "linked"]),
  type: z.string().min(1),
  /** AgentScript expression emitted after `=` without string coercion. */
...
export const BrokerActionSchema = z.object({
  ...
  /**
   * Legacy action-definition headers retained only so old saved drafts can be
   * loaded and migrated. AgentFabric accepts http_headers as an invocation
   * binding, not as a top-level action field; serializers must never emit this.
   */
...
export const LlmBindingSchema = z.object({
  ...
  /** Provider request options from the AgentFabric dialect. */
...
export const GraphNodeSchema = z.object({
  ...
  /** Preserve imported single-line `instructions: -> <expression>` procedure form. */
  reasoningInstructionsProcedureInline: z.boolean().optional(),
  ...
  /** Preserve imported single-line `prompt: -> <expression>` procedure form. */
  promptProcedureInline: z.boolean().optional(),
  ...
  /**
   * Single on_exit transition target (node id) for non-router nodes and for
   * the trigger's on_message. Router encodes targets in routes/otherwise.
   * Artifact/progress echoes use on_exit to continue to a terminal status echo.
   */
...
export const BrokerSchema = z.object({
  ...
  /** Preserve imported single-line `system.instructions: -> <expression>` procedure form. */
  systemInstructionsProcedureInline: z.boolean().optional(),
  ...
  agentScriptVariables: z.array(AgentScriptVariableSchema).default([]),
...
export type AgentScriptVariable = z.infer<typeof AgentScriptVariableSchema>;
